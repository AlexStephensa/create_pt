...
class ScoringNotifier extends StateNotifier<ScoringState> {
  ...
  void recordShot(String result) {
    ...
    newShots[state.currentShooterIndex] = List<String>.from(
      newShots[state.currentShooterIndex],
    )..add(result);
    ...
    } while (newShots[nextIndex].length == 25 &&
        !newShots.every((s) => s.length == 25));

    state = state.copyWith(shots: newShots, currentShooterIndex: nextIndex);
  }
  ...
  void undoLastShot() {
    ...
    state = state.copyWith(shots: newShots, currentShooterIndex: prevIndex);
  }
    ...
    for (int i = 0; i < state.shooters.length; i++) {
      ...
      for (var shot in state.shots[i]) {
        if (shot == 'hit') {
          ...
          hits += 0;
          misses += 1;
        } else if (shot == 'dead_pair') {
          hits += 2;
          misses += 0;
        } else if (shot == 'dead_loss') {
          hits += 0;
          misses += 2;
        } else if (shot == 'loss_pair') {
          hits += 1;
          misses += 1;
        }
      }
